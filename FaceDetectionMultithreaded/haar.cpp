...
void ScaleImage_Invoker( myCascade* _cascade, float _factor, int sum_row, int sum_col);
    ...
    for( factor = 1; ; factor *= scaleFactor )
        {
            ...
            MySize sz = { int(float(img->width)/factor), int(float(img->height)/factor) };
...
void doThreadWork(int maxX, myCascade* _cascade, float _factor, int lowerBound, int upperBound, MySize winSize, int totalSteps)
{
	int result;
	MyPoint p;
	myCascade* cascade = _cascade;
...
void ScaleImage_Invoker( myCascade* _cascade, float _factor, int sum_row, int sum_col)
{
	...
	int calcsPerStep = steps / numthreads;
	...
	for (int i = 0; i * calcsPerStep < steps; i++)
	{
		threads.push_back(std::thread(doThreadWork, x2, cascade, factor, i*calcsPerStep, (i + 1)*calcsPerStep, winSize, steps));
	}
  ...
  / (in the 5kk73 example, there are 25 stages)
    **************************************************/
    ...
    //printf("\n total_nodes = %d", total_nodes);
        ...
        scaled_rectangles_array = (int **)malloc(sizeof(int*)*total_nodes*(12+0));
