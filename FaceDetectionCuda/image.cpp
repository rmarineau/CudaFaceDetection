...
int readPgm(char *fileName, MyImage *image)
{
    ...
    //fseek(in_file, position-3, SEEK_SET);
    ...
    image->maxgrey = atoi(mystring);
        printf("\n ---> w=%d h=%d", image->width, image->height);
        ...
        for(i=0;i<(image->height*image->width);i++)
            {
                ch = fgetc(in_file);
                    if (ch != EOF) {
                        ...
                        image->data[i] = (unsigned char)ch;
                            //printf(".");
                                //printf("\nERROR: reading input image i=%d (of max %d)\n",i,image->height*image->width);
...
void createImage(int width, int height, MyImage *image, cudaStream_t thisStream)
{
    image->width = width;
    image->height = height;
    image->flag = 1;
	cudaMallocManaged(&(image->data), sizeof(unsigned char)*height*width);
	//cudaDeviceSynchronize();
	cudaStreamAttachMemAsync(thisStream, image->data);
    //image->data = (unsigned char *)malloc(sizeof(unsigned char)*(height*width));
}

void createSumImage(int width, int height, MyIntImage *image, cudaStream_t thisStream)
{
    image->width = width;
    image->height = height;
    image->flag = 1;
	cudaMallocManaged(&(image->data), sizeof(int)*height*width);
	//cudaDeviceSynchronize();
	cudaStreamAttachMemAsync(thisStream, image->data);
